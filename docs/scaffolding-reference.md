# Nexus Brain — Scaffolding Reference
...
This project runs the **agent-team orchestrator + implementer pattern** (three roles + human). Same slash commands, same Step-9 routing matrix, same N+2 commit cadence, same cross-doc invariants discipline as the universal pattern. **Adaptations are project-shaped:** this project's stack, code areas, phase plan (`Phase <N> (tasks <N>.<M>)`), forbidden patterns, and architecture are its own.
...
N slice commits + 1 session-doc commit + 1 round commit = **N + 2** per round. Push once at `/orchestrate-end` — **only when a remote is configured** (to **origin (git@github.com:SiWarlock/Nexus-Brain.git)**).
...
- **Forbidden patterns:** see `core/CLAUDE.md`.
- **Key safety rules:** see root `CLAUDE.md` "Key safety rules."
- **Cross-doc invariants:** the `core/CLAUDE.md` table tracks typed models mirroring `ARCHITECTURE.md` sections. Field changes require atomic doc edits.
...
| Conventions / patterns | `core/LESSONS.md` (prose) + `core/CLAUDE.md` (index) | On-demand |
...
- **New lesson** → next anchor in `core/LESSONS.md` + row in the `core/CLAUDE.md` index. Hot-routed at Step 9.
- **New convention** → entry in `core/CLAUDE.md` Forbidden patterns or root `CLAUDE.md` Key safety rules + a `LESSONS.md` entry if durable.
- **New cross-doc invariant** → row in the `core/CLAUDE.md` table + atomic `ARCHITECTURE.md` edit.
...
**Don't** add project *state* to scaffolding files — state lives in `IMPLEMENTATION_PLAN.md`. **Don't** rename the cross-referenced files (`IMPLEMENTATION_PLAN.md`, the `CLAUDE.md` files, `core/LESSONS.md`, `docs/team-protocol.md`, `docs/orchestrator-briefing.md`, `docs/tdd-brief-template.md`) casually — they're named inside slash command bodies; renaming is a multi-file ripple.
