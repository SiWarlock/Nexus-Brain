# project-brain — Research Dossier (Appendix)
...
**Recommended:** an **MCP-tool-first agentic harness.** Expose the index as a **local MCP server** (FastMCP 3.0, Python, Jan 2026 — decorator API, OTel, granular auth; or the TS SDK) over **stdio**. The server is the trust boundary and the only thing touching raw code.
...
| **Task tracker** | `MVP_TASKS.md` (root) | `tasks-gen` | Phases carry `**Spec anchors:** ARCHITECTURE.md §X`; dense checkbox tasks with `Files:`, `Cross-doc invariant:`. |
...
| **Provenance** | `.scaffolding/manifest.json` | `scaffold-generate` | `schemaVersion 1`; `scaffoldingRepo`, `generatedFromSha`, `placeholders{}`, `codeAreas[]`, `generatedFiles[{dest,template,kind}]`, `exampleBlocks[]`. Machine-owned, committed, never hand-edited. |
...
arch-draft → arch-finalize → tasks-gen → scaffold-generate → /tdd ─┐
        ...
        ▼                  ▼
  learn-site ◀──── [ project-brain ] ──── /ask, federation MCP
...
- `/Users/dreddy/Documents/Dev/AI-tools/claude-code-tdd-agent-crew-scaffolding/templates/MVP_TASKS.md`
