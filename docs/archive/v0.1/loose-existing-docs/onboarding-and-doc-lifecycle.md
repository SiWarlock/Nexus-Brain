...
> Source: user direction (2026-06-03), to be folded into `PRD.md`. These add an **Installation, setup &
> onboarding** section and a **Documentation lifecycle / freshness loop** requirement, and they sharpen the
> cc-crew boundary (skills bundling).

## R-INSTALL — one-command global install that bootstraps EVERYTHING
Installable as a global package / CLI (e.g. `npm i -g project-brain` or `npx project-brain`). A single
`project-brain setup` (run once per machine) makes it ready out of the box by ensuring **all** moving parts:
1. **CodeGraph present.** project-brain treats CodeGraph as a dependency and ensures it's installed +
...
2. **MCP server configured + installed.** Register the project-brain MCP server into the host configs
...
3. **Skills registered.** Auto-register the doc skills it needs (`layer-docs`, `learn-site`, and any new
   doc-update skill) into `~/.claude/skills` and `~/.codex/skills`.
4. **Central store created** (`~/.project-brain/`) + the project registry.
**Boundary decision this forces:** for a fresh machine with ONLY project-brain to work, project-brain must
**bundle or fetch the skills it needs** (vs. depend on a cc-crew checkout). Options: (a) extract the
doc-skills into a shared installable unit both repos use; (b) project-brain vendors its own copy/fork of the
doc skills; (c) hard dependency on cc-crew. Lean (a) or (b) so install is self-contained.

## R-ADD — `project-brain add` bootstraps the WORKSPACE
Running `project-brain add` inside a repo should set up everything that project needs, automatically:
...
1. **Doc regeneration (cc-crew side):** `layer-docs` (and friends) need an `--update` / refresh mode that
   incrementally regenerates docs for *changed* areas in place (not a full rewrite). New cc-crew capability.
...
pipeline**: detect staleness → trigger `layer-docs --update` → re-embed. A self-updating project brain. Strong
differentiator; call it out as a signature feature.
