...
- **Orchestrator doc rows to write hot (Step 9 routing):** <none / which `core/CLAUDE.md` cross-doc rows + `ARCHITECTURE.md` Appendix A rows the orchestrator authors atomic with the round>
...
> **Orchestrator territory** (canonical list: the `core/CLAUDE.md` "must NOT touch" list — hook-enforced in team mode): flag at Step 9 categorized; the orchestrator writes hot during the same session and commits at `/orchestrate-end`.
