...
**State machines (all enumerated; Appendix A):** Index generation (`building → reembedding → validating → swapping → active(+retired→gc_eligible→purged)`; failure edges discard new, keep old; ENOSPC pre-flight) · Anchor (`live ⇄ stale|moved|unknown → orphaned`; recovery edges back to `live`; **`AnchorState` = these 5 states — `deleted` is anchor-record removal/lifecycle, NOT a `state` value**, frozen @`5b50b5f`) · Project (`added → indexing → ready ⇄ syncing → drift_detected → reindexing → (index_failed|archived|removed)`) · Worker (`cold → warm → idle_evicted`; `write_held(lease)`; crash→reattach) · EpisodeCard (`no_consent → consent_granted → reading → redacting → (quarantined_unsafe|redacted) → summarizing → embedded → linked`; terminals `consent_revoked`(purge), `superseded`) · Doc-refresh (ownership-gated, consent+conflict) · WorkflowInstance (12 frozen R-7 states; transition graph co-designed w/ NexusOps — LBD-12). **Manifest/registry schema is FROZEN (closes OQ-6)** with a forward-only `schemaVersion` migrator + backup-before-migrate + downgrade-refuse (D-26/C-12).

## §6 — LanceDB store & maintenance contract

LanceDB (pinned; high-level API pre-1.0 on stable Lance SDK 1.0/format 2.1). **One dataset per project** (isolation, idle-eviction, per-project blue-green). **Maintenance contract `[PH]` (mandatory, D-25):** `optimize()` after each upsert batch + monitor `index_stats().num_unindexed_rows ≈ 0` (post-write rows fall to flat scan; `fast_search` silently excludes them); scheduled `cleanup_old_versions()`; **git-SHA version tags (GC-exempt → double as §5's canonical SHA)**; **single-writer-per-dataset** (lease) + read-only federation; RAM-bounded batched index builds; pin `lancedb`; **`spawn` not `fork`**; verify arm64 wheels in CI; **ENOSPC pre-flight before blue-green** (abort new generation, retain prior, surface remediation). FTS caveat: no boolean AND/OR in the native query string → model boolean logic in the retrieval layer; `with_position` only for phrase queries. **[O-LANCE-BAKEOFF rig, spike 0.4]** the reusable maintenance-contract measurement RIG (`ci/bench/lancedb_maintenance/`) reports `optimize()` latency · index-build RAM · steady-state disk for the "invisible" budget; its PROPOSED ceilings (optimize ≤5000ms · index-build RAM ≤2GB · steady-state disk ≤5GB + a `num_unindexed_rows==0` post-optimize monitor folded into the gate) are placeholders pending the **authoritative Phase-3 real reference-Mac run** (D-A17 scope B — deferred, not dropped). Fold the real ceilings into this contract / the perf baseline when Phase 3 measures them.
...
> **[Phase-2 impl note · 2026-06-21]** The chunk stage emits an internal `ChunkDraft` (the chunk-derivable subset of `Chunk` + anchor span data; the frozen `Chunk`/`Anchor` are assembled at 2.4 — both all-required: ids/SHA/vector). **Docs:** ATX heading-split (fenced-code-immune), cap-bounded line-tiling. **Code:** AST via the **vendored MIT `CodeHierarchyNodeParser`** (`core/ingest/_code_hierarchy.py` — the pip pack `llama-index-packs-code-hierarchy` is deprecated/un-importable on 3.12; owner-decided vendor, driven by **standard `tree_sitter` + individual `tree-sitter-<lang>` grammars**, not the bundled pack). Code mapping = leaf full nodes (verbatim text + accurate span + qualified `Class.method` symbol) + line-tiled complement; skeleton/parent nodes skipped → **anchor accuracy by construction** (every chunk's text == the source slice at its span, §10). Every draft ≤ `TEXT_MAX_LEN`. v0 wires **4 of the 12 ratified languages with AST hierarchy** (Python · TypeScript · C++ · PHP — a parser-config ceiling; extension JS/Rust-first is a tracked follow-up); all other languages + parse-error/oversized(>2MB) → **line-tiling fallback** (full coverage, no hierarchy). `register`=`plain` v0 (`deep` reserved for context-augment). `chunker_version` provenance stamped at 2.4.

> **[Phase-2 impl note · 2.4+2.S · 2026-06-21]** The `add` orchestration (`core/ingest/pipeline.py::add`, `2b36fad`) composes discover→classify→chunk→**redact(persist)**→embed→**assemble frozen `Chunk`+`Anchor`**→**write the manifest via `HostPort.perform`** (the first `core/` FS mutation — Key-safety #4). **Phase-2 scope:** chunks are assembled + redacted against **`Fake*` providers** and the manifest is the only persisted artifact; **LanceDB embed/persist + the real git-SHA version tag are Phase 3.1** — a deterministic v0 `recipe_sha` (sha256 over sorted `(path, raw-content-hash)`) stands in for `ingested_from_sha`/`lance_version_tag`/`last_resolved_sha`. **Idempotency** keys on `ManifestArtifact.content_hash = sha256(raw file bytes)` (per-file; chunk-level tombstone+replace = Phase-3/§12). **Content sanitization (the consuming phase, §10/LESSON 16):** NUL/C0/DEL files trip the per-file **R-PARTIAL** boundary → skip (never abort the repo); **Trojan-Source bidi + zero-width are STRIPPED pre-chunk** — so **anchors are accurate against the SANITIZED source, not the raw file** (the index carries the true logical order, not the visual reorder); **Phase-4 grounding/anchor-revalidation must revalidate against the sanitized source (or re-sanitize on read).** Two distinct content hashes (a Phase-3 consumer must NOT conflate): **`Chunk.content_hash`** = hash of the redacted chunk text · **`ManifestArtifact.content_hash`** = hash of the raw source file (the idempotency key). The §14 mutation chokepoint covers **WRITES only**; the ingest pipeline **READS** source files directly (reads are outside `HostPort` scope). **Residual:** parent-dir-swap TOCTOU in `StandaloneHost` containment → **Phase-8.2 carry** (same-uid = §18/LBD-13 non-goal).
...
**KEEP-OUR-ROUTER (HYBRID-lean, D-27).** Router reads N per-project LanceDB datasets **read-only** + queries N CodeGraph DBs via `CodeGraphPort`; **union + RRF rank-fusion**; gates each store on its **own** stamp (registry = routing index only, §5). **Result shape carries `{projects_requested, answered, excluded[]}`** (a silently-partial portfolio answer is the federation analogue of the trust failure). **Cross-repo symbol resolution** via `unresolved_refs` + namespaced `qualified_name` = `[SPIKE — O-FED ✓ retired 2026-06-20: VIABLE as a uniqueness-gated merge enrichment — merge only on a unique segment-suffix match vs is_exported nodes, NO heuristic tie-breaking; side-by-side-marked stays the default. Synthetic precision 1.000/recall 0.967; 2 HIGH real-data caveats (sparse/empty real unresolved_refs caps recall; thin real qualified_name → homonyms, precision preserved); real-CodeGraph validation deferred to provisioning. Router design decision → Phase-6. See ci/probes/federation_spike.md]`; fallback = side-by-side-marked. **Hybrid-lean:** where repos already nest under one root, treat CodeGraph native co-indexing as one source; everywhere else the router fans out. Optional in-process backing: the DuckDB Lance core extension. On-demand workers + LRU idle eviction; only the router is always-on.
...
> **[Phase-2 impl note · 2026-06-21]** The catchable-set redaction ENGINE landed (`core/ingest/redactor.py`, `CatchableSetRedactor` implementing the frozen 1.5 `Redactor`): PEM / prefix-token / JWT + URL-credential + assignment-value passes; the passthrough **allowlist (git-SHA/ULID/UUID) runs BEFORE the entropy test** (cardinal residual protection — a SHA redacted breaks the LanceDB version tag); contextual entropy (assignment values only → 0% FP); fixed secret-free `[REDACTED_*]` markers with **exact-match idempotence** (a `startswith` check is input-spoofable); **fail-CLOSED** on any internal error (returns a marker, never the original). **D-A5/D-A6 RESOLVED (owner-ratified):** the catchable-set envelope = **recall ≥0.95 / FP ≤0.05 / git-SHA FP =0%, UNIFORM across all three sinks** (cloud-stricter reserved as a future additive branch; the iface stays sink-parameterized). The §0.1 fuzz harness is wired as a hard CI gate (`ci/eval/redaction_fuzz/`, `ci` imports `core` never the reverse): recall 1.000, FP 0.000, git-SHA FP 0%, all three sinks PASS. **Known gate-coverage gap:** the synthetic corpus doesn't yet exercise comma/brace-embedded + URL-under-sensitive-key shapes (the unit tests do) — corpus extension tracked. The authoritative real-repo recall/FP recalibration is deferred to real ingest (LESSON 18). CONTENT sanitization (Trojan-Source bidi + NUL/C0/DEL) is a 2.4/sibling concern, NOT in the secret engine (LESSON 14).
...
| **Anchor** | §10 | anchor_id·project_id·source_file·source_span·target_path·target_line_start/end·target_symbol?·state(live\|stale\|moved\|unknown\|orphaned)·last_resolved_sha·confidence (11 fields, frozen @track/contract `5b50b5f`; `state`=`AnchorState` StrEnum, 5 values — **`deleted` is anchor-record lifecycle, NOT a `state` value** (§5 line-90 reconciliation); invalid state rejected at parse; `target_symbol` optional; `confidence`∈[0,1]; identity strings strip+min_length) | ★ |
| **ProvenancePacket + EvidenceRef** | §10 | ProvenancePacket (10 fields, frozen @track/contract `77276e3`): project_ids·source_ids·citations(file:line[] `tuple[str]`)·commit_shas·session_ids·recorded_sha?·index_freshness·confidence·drift_markers·**evidence: tuple[EvidenceRef]** (+evidence[] additive — **D-A12**; the ER-map mandates the aggregation; `low_confidence_links` is NOT a field — Phase-4 gate output). EvidenceRef (4 fields, frozen `518da07`) `{type∈EvidenceType, label, resource_ref?, confidence?}`; **EvidenceType membership DEFERRED to Phase-4 grounding — D-A11** (canonical 11 = NexusOps `MAIN_PLATFORM_INTERFACE.md` v0.2; constrained-str alias now, narrow additively, no membership snapshot yet). | ★ |
| **HostPort** | §7 | `capabilities()→frozenset[HostCapability]`·`authorize(intent)→HostAction`·`perform(action)→HostResult` (frozen @track/contract `8aa6935`). `HostCapability` closed StrEnum `{own_store_write\|owned_doc_refresh\|consented_host_config}` (membership snapshot). **Fail-closed authorize** (∉caps→`HostDenied`); **`perform` defense-in-depth re-validates capability∈caps** (forged `authorized` flag still denied). Intent/Action carry optional **`payload: StoreWritePayload \| None` `{rel_path: IdentityStr, content: bytes}`** (Option C, owner-ratified @ `20ac95d` — additive/backward-compatible, absent→None; widen to a per-capability union at 3.x); Result minimal `{ok, detail?}`. **Real `StandaloneHost` LANDED** (2.S `20ac95d`: atomic `mkstemp`+`os.replace` write, realpath containment, default `capabilities()={own_store_write}`, fail-closed authorize/perform + unhandled-cap fail-close). §14 INV-allowlist **FULL runtime proof LANDED** (2.S/D-A13: runtime per-mutator proof + hardened static tripwire — module-aliasing/getattr/session-state). **Residual: parent-dir-swap TOCTOU → Phase-8.2 carry** (same-uid = §18/LBD-13 non-goal). NexusOpsHost→ActionPlan (P2). | ★ |
| **Provider ports** | §7/§16 | EmbeddingProvider`{embed,dimension,model_version}`·Reranker`{rerank}`·ContextStrategy`{augment}`·ModelProvider`{generate+citations}` (4 behavioral Protocols + Fakes, frozen @track/contract `e9d1e51`). `model_version` on EmbeddingProvider ONLY (load-bearing §5 generation identity; reranker/model versions = eval/obs concerns). Frozen result types `RerankResult{index,score}`(allow_inf_nan=False)·`Citation{cited_text,source_index}`·`GenerateResult{text,citations}`; rich Citations payload Phase-4-deferred; **cassette record/replay re-sequenced to providers/eval** (eval-infra, not a freeze contract). | ★ |
| **CodeGraphPort** | §7/§11 | `query(kind: CodeGraphQueryKind, sym)→CodeGraphResult` (CLI shell-out; frozen @track/contract `b1dafcc`)·5 tables (nodes/edges/files/unresolved_refs/project_metadata)·**schema_versions assert `MAX≥5`** (NOT `=1` — spike-0.2/**D-A4**)·`CODEGRAPH_DIR` **positive allow-list** resolver (LESSON 10)·tree-sitter fallback; pin `=1.0.1`. `CodeGraphQueryKind`{explore,node,callers,callees,search}; **`search`→CLI `query`** (no `codegraph search`). Real CLI adapter + version-gate(`≥1.0.1`/MCP-route) + output parse = Phase-4.2; **argv-hardening (`shell=False`+`--`) = Phase-4.2 ★SAFETY / D-A14** (primary query-arg injection defense). | ★ |
...
| **EventSource** | §7 | `{poll()→tuple[Event,...], subscribe(handler)}` + `Event{kind, source}` (frozen @track/contract `05c3551`; +Fake). Git-hook/watcher standalone; NexusOps-outbox + EventEnvelope variant = P2. _(Appendix-A row additive — D-A12-class reconciliation; §7 names all 11 ports Appendix-A contracts.)_ | ★ |
| **SecretStore** | §7/§18 | `{get_ref(name)→SecretRef, resolve(ref)→str}` + `SecretRef{service, account}` (frozen @track/contract `05c3551`; +Fake). **Key safety rule #3:** SecretRef carries NO secret (`extra="forbid"` rejects secret/value/password/token; no plaintext in ref/repr/str/dump/logs); `resolve` fail-closed → `SecretNotFoundError` (§25). Real keychain adapter + log-scrub = Phase-2. _(Appendix-A row additive.)_ | ★ |
| **ObservabilitySink** | §7/§19 | `{emit(event: ObsEvent)}` + `ObsEvent{name, attributes: tuple[(str,str)]}` (frozen @track/contract `05c3551`; +Fake records locally). **Instrumented-but-silent: off-by-default + local-only + NEVER phone home** (D-22). Real OTel sink = Phase-2. _(Appendix-A row additive.)_ | ★ |
| **MCP tool contract** | §14 | search/get_file/graph/list_projects/status: params(`RetrievalScope`{project,portfolio}·project_id·top-k, bounded `MAX_TOP_K`/`MAX_QUERY_LEN`)·**`get_file` ASCII positive allow-list** (LESSON 10; runtime realpath-containment Phase-8.2)·result(`McpResultItem` chip·file:line·ids + `ProvenancePacket`, ≤`MAX_RESPONSE_ITEMS`)·streaming·**`PolicyDenied` marker** (`McpToolResult` union, marker-not-error)·ingress validation; frozen @track/contract `e65b9e3`+`959d5d9` (LESSON 14/15) | ★ |
| **policy.yaml** | §16 | `schema_version`·providers·privacy(local\|cloud)·MCP-boundary filter·federation visibility·session consent·`.brainignore`; frozen `ProjectPolicy` **fail-CLOSED** (most-restrictive defaults; StrictBool opt-ins); frozen @track/contract `a9df580` (LESSON 13) | ★ |
| **Redactor** | §18 | `redact(payload, sink∈{persist,mcp_egress,cloud_egress})` Protocol + `FakeRedactor`; catchable-set recall floor (≥95% / FP≤5% / git-SHA 0%) + 3 accepted residuals + fuzz corpus (SSoT = spike `harness.py`, enforced Phase-2.3); behavioral-invariant freeze (idempotent / never-raises / git-SHA-passthrough); frozen @track/contract `343b6fb` (LESSON 12) | ★ |
