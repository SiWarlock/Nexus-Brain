...
| **HostPort** | §7 | `capabilities()→frozenset[HostCapability]`·`authorize(intent)→HostAction`·`perform(action)→HostResult` (frozen @track/contract `8aa6935`). `HostCapability` closed StrEnum `{own_store_write\|owned_doc_refresh\|consented_host_config}` (membership snapshot). **Fail-closed authorize** (∉caps→`HostDenied`); **`perform` defense-in-depth re-validates capability∈caps** (forged `authorized` flag still denied). Intent/Action/Result minimal frozen shapes (per-capability payloads + real `StandaloneHost` Phase-2). §14 INV-allowlist **static tripwire seeded**; FULL runtime proof = Phase-2 (**Task 2.S / D-A13**). NexusOpsHost→ActionPlan (P2). | ★ |
