...
| `HostPort` (+ `HostCapability`, `HostIntent`/`HostAction`/`HostResult`, `HostDenied`) | §7 (Appendix A) | **The SOLE mutation chokepoint (Key safety rule #4 / §4 #3).** `@runtime_checkable Protocol`: `capabilities()→frozenset[HostCapability]` · `authorize(intent)→HostAction` · `perform(action)→HostResult`. `HostCapability` = closed 3-value `StrEnum` `{own_store_write, owned_doc_refresh, consented_host_config}` (membership snapshot, LESSON 6). **Fail-closed `authorize`:** capability ∉ `capabilities()` → `HostDenied` (empty allowlist denies all). **`perform` defense-in-depth:** re-validates capability ∈ `capabilities()` AND the `authorized` stamp — a forged `authorized=True` for a non-allowlisted capability is still denied (LESSON 9). Intent/Action carry optional **`payload: StoreWritePayload \| None` `{rel_path: IdentityStr, content: bytes}`** (Option C, owner-ratified @ `20ac95d` — additive, absent→None backward-compatible; widen to a per-capability union at 3.x); Result `{ok, detail?}`. **Real `StandaloneHost` LANDED (2.S `20ac95d`):** atomic `mkstemp`+`os.replace` write + realpath containment; default `capabilities()={own_store_write}`; an allowlisted-but-unhandled capability fails closed (`HostDenied`). **§14 INV-allowlist FULL runtime proof LANDED (Task 2.S / D-A13):** runtime per-mutator proof + hardened static tripwire (module-aliasing/`getattr`/session-state `os.environ`/`putenv`) over all `core/` except `host.py`. **Residual: parent-dir-swap TOCTOU → Phase-8.2 carry** (same-uid = §18/LBD-13 non-goal). `FakeHost` (testing/fakes.py) = same fail-closed contract (LESSON 1). Pinned by `test_host.py`. |
...
| 24 | 2026-06-21 | [Runtime per-mutator proof = recording host + hardened static scan](LESSONS.md#24) | The runtime upgrade of a static mutation-chokepoint tripwire: a recording host adapter proves the only FS write routes through `perform`, AND the static AST scan is hardened against the bypasses a static-only check misses — module-aliasing (`import os as _o`), `getattr`/dynamic dispatch, and session-state writes (`os.environ`/`putenv`/`unsetenv`). The two together make it a per-mutator proof, not a name-match. _(pin: `core/tests/ports/test_host.py` runtime-proof + hardened-tripwire (aliasing/getattr/session-state) + `core/tests/ingest/test_pipeline.py::test_add_routes_all_mutation_via_perform`)_ |
| 25 | 2026-06-21 | [Grow a frozen safety contract additively — optional field, absent→None](LESSONS.md#25) | When a frozen ★safety contract must gain a field (here a per-capability `HostIntent`/`HostAction` payload), add it OPTIONAL defaulting to the absent case (`payload: X \| None = None`) so every existing wire shape still validates (backward-compatible) and every schema-snapshot stays green except the one intended field-set; widen a single-case payload into a per-capability union additively when later cases land. Doesn't weaken the invariant — `perform` still re-validates the allowlist. _(pin: `core/tests/ports/test_host.py` StoreWritePayload/HostIntent/HostAction schema-snapshots; existing intents validate with `payload=None`)_ |
