...
## <a id="24"></a>24. The runtime per-mutator proof = a recording host + a hardened static scan (a static tripwire upgraded)

**Date:** 2026-06-21.
**Source slice:** 2.S (`add_ingest_pipeline_and_hostport_runtime_proof`) — the cardinal Key-safety-#4 proof.

A static AST tripwire ("no module names an FS/git mutation primitive outside the allowlisted file") is necessary but NOT sufficient as the §14 INV-allowlist proof — a static name-match is blind to (a) **module-aliasing** (`import os as _o; _o.remove(...)`), (b) **`getattr`/dynamic-dispatch** resolution (`getattr(os, name)(...)` — when `name` is dynamic the scanner can't prove it safe, so flag conservatively; a constant attr is checked against the mutator set), and (c) **session-state** writes that aren't "fs/git" but ARE the "external/session state" the safety rule names (`os.environ[k]=`, `.update`/`.pop` on `os.environ`, `os.putenv`/`os.unsetenv`). Hardening the scan against those three closes the 1.4a residual. The RUNTIME half is a **recording host** (a `StandaloneHost` subclass that records every `perform`): drive the real mutator (here `add`'s manifest write) and assert the only FS write that happened went THROUGH `perform` (and the file landed). The two halves together — hardened static "no escape" + runtime "the one real write is via perform" — make it a per-mutator proof, not a name-match. The real adapter's `perform` also re-validates the allowlist AND fails closed on an allowlisted-but-unhandled capability (don't trust a forgeable `authorized` stamp; don't half-execute a capability with no handler).

**Rule:** Upgrade a static mutation-chokepoint tripwire to a runtime per-mutator proof with TWO halves: harden the static AST scan against module-aliasing + `getattr`/dynamic-dispatch + session-state writes (not just fs/git), AND add a recording-host runtime test that the real flow's only mutation routes through `perform`. A green static name-match alone is not the proof.

## <a id="25"></a>25. Grow a frozen safety contract additively — an optional field defaulting to the absent case keeps every existing wire shape valid

**Date:** 2026-06-21.
**Source slice:** 2.S (§7 `HostPort` payload growth, owner-ratified Option C).

When a frozen ★safety contract must gain a field after the freeze (here `HostIntent`/`HostAction` needed a per-capability write `payload` so `perform` could carry the manifest path+content — the freeze had deferred payloads), grow it ADDITIVELY: add the field OPTIONAL with the absent case as the default (`payload: StoreWritePayload | None = None`). Then (a) every existing wire shape still validates (a contentless intent is backward-compatible — `payload` defaults to `None`), (b) every schema-snapshot stays green except the ONE intended field-set (the change is visible + reviewed; nothing else moves), and (c) the safety invariant is untouched — `perform` still re-validates the allowlist; the payload describes WHAT to mutate, not WHETHER it's allowed. Shape a single-case payload (one capability's `{rel_path, content}`) so it widens into a per-capability discriminated union additively when the other capabilities land — never a breaking re-shape post-fork. Growing a frozen ★ cross-track contract is still a load-bearing decision (owner-ratified) + a cross-doc invariant write (Appendix-A row + the area cross-doc table) + a cross-track Finding — but additive + backward-compatible means no parallel track breaks (they pick it up on their next merge).

**Rule:** To grow a frozen safety contract, add the field OPTIONAL defaulting to the absent case (`X | None = None`) — existing wire shapes validate unchanged, snapshots stay green except the intended field-set, the invariant is preserved (the re-validation still runs); widen single→union additively. Still owner-ratified + cross-doc-written + a cross-track Finding, but additive ⇒ no consumer breaks.
