...
**Track `contract`, Phase 1.** Phase-0 spikes 0.1 (redaction envelope) + 0.2 (CodeGraph coldiff) ✓ landed (0.3/0.4 not-yet-run, 0.5 HITL-deferred — all before Phase-0 exit). **Phase 1.1 (determinism seams) ✓ and Phase 1.2 (the §5 data + migration contracts) ✓ COMPLETE.** Last commit `bb000b1` (track/contract; suite **82** green, mypy --strict + -W error clean). Not pushed beyond the round push. LESSONS §1–§5 banked. **Handoff doc: `docs/sessions/contract-001-2026-06-17-phase1-contracts.md`** (read first).

**Next session target:** **Phase 1.3 — trust contracts** (`Anchor`/`ProvenancePacket`/`EvidenceRef` → `core/model/{anchor,provenance,evidence}.py`; §2.5-seam schema-snapshots `spec(§10)`; mirrors the §5-model conventions + LESSONS §1–§5). Then 1.4 (ports incl. HostPort + CodeGraphPort) → 1.5 (MCP contract · policy.yaml · Redactor iface). Fold the triaged Carry-forward (esp. the **before-fork whitespace sweep** + the spike-0.1/0.2 inputs for 1.5/1.4). After 1.5 + the sweeps + Phase-0 0.3/0.4(/0.5) → `/phase-exit 1` (the fork gate).

---

## Carry-forward to upcoming briefs

> _(Triaged at end-of-1.2 cycle. Full organized open-items in the handoff doc `docs/sessions/contract-001-2026-06-17-phase1-contracts.md`.)_

- **1.4 `CodeGraphPort`** *(spike 0.2 / D-A4; `ci/probes/codegraph_coldiff.md`)* — pin `=1.0.1`; assert `schema_versions MAX >= 5` (NOT `=1`); `search` kind → CLI `codegraph query`; version-check + fail-fast if `< 1.0.1` (system is v0.9.7, lacks explore/node) OR route via the 1.0.1 MCP daemon. Reconcile the §0.2/Appendix-A `schema_versions` value when authoring 1.4.
- **1.5 `Redactor` + policy** *(spike 0.1; `docs/audits/redaction-envelope.md`)* — freeze `redact(payload, sink: Sink) -> str`, `Sink`={persist,mcp_egress,cloud_egress}; docstring enumerates the 3 accepted residuals (§18/C-11); envelope recall ≥95%/FP ≤5% (git-SHA 0% FP); invariants idempotent+never-raises+git-SHA-passthrough. **FLAG-4 (cloud stricter?) + the 95/5 threshold OWNER-deferred to 2.3/policy.yaml (D-A5/D-A6)** — signature accommodates both; don't block. **policy model = fail-CLOSED** (absent/empty/unrecognized → most-restrictive local-only).
- **Identity-field whitespace-strip uniform sweep — MUST-do-BEFORE-FORK** *(1.2c1; [sec low])* — `min_length=1` admits `"   "`. Apply `StringConstraints(strip_whitespace=True, min_length=1)` UNIFORMLY across all §5 identity fields: retrofit 1.2b/1.2c1/1.2c2 + bake into 1.3 from the start. Whitespace-loose identity in a frozen cross-track contract post-fork = a Finding.
- **On-disk strict key-shape rejection → Phase-2/3 startup-reconcile loader** *(1.2c1)* — the frozen models are lenient readers by design (`validate_by_name` accepts snake OR camel; dup-key resolves to alias). Strict wrong/duplicate-key rejection is the loader's job (1.2d turned out PURE, no loader). Documented by `test_manifest` lenient-read test.
- **Phase-2.3 redactor engine** — wire `ci/eval/redaction_fuzz/` as a CI hard gate; address FLAG-1/2/3 (encoding-aware oracle · JWT shape-matcher · binary-artifact corpus). Detail in the envelope doc §8.
- **Standing scaffolding/owner items** — (a) spec-lint numeric-ID fix `392ed4f` → carry upstream at `/scaffold-upgrade`; (b) `requires-python "<3.13"` deliberate bump when moving to 3.13; (c) **D-A3 `.claude/commands/preflight.md` Step-4 `mypy core`→`mypy .`** (HITL-deferred, UPGRADED to recommended-FIRST owner action — root enabler of hand-rolled gates; implementers override with `mypy .`).
