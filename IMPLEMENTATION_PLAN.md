...
**Track `contract`, Phase 1.** Phase-0 spikes 0.1 + 0.2 ✓ (0.3/0.4 not-yet-run, 0.5 HITL-deferred — before Phase-0 exit). **Phase 1.1 ✓ · 1.2 ✓ · 1.3 (trust contracts: Anchor/EvidenceRef/ProvenancePacket) ✓ · 1.4 (all 11 ports frozen) ✓ COMPLETE.** Last commit `05c3551` (track/contract; suite **174/174** green, mypy --strict + ruff clean). **Cycled at end-of-1.4** (fresh full-runway team takes the fork-gate stretch). LESSONS §1–§11 banked. **Handoff doc: `docs/sessions/contract-004-2026-06-18-phase1-trust-and-ports-handoff.md`** (read FIRST — has the D-A* fork obligations + before-fork checklist) + the decision log `docs/lead-decisions-while-away.md` (D-A1–A14) + the impl session doc `contract-003-*`.

**Next session target:** **Phase 1.5 — boundary contracts** (`MCP tool contract` · `policy.yaml` · `Redactor` interface → `core/model/{mcp_contract,policy,redactor_iface}.py`; spike-0.1 envelope inputs; **LESSON 10 positive allow-list applies to MCP `get_file` ingress validation**; policy = fail-CLOSED; Redactor signature accommodates the D-A5/D-A6 owner-deferred strictness/threshold). Then the **before-fork hardening sweep** (Carry-forward — whitespace-strip retrofit + IdentityStr consolidation + list→tuple + control-char/NUL) + **Phase-0 spikes 0.3/0.4** (0.5 HITL-deferred) → `/phase-exit 1` (the fork gate; runs the arch-drift audit + the D-A13/D-A14 handoff flags into the contract→spine handoff) → merge to integration.

---

## Carry-forward to upcoming briefs

> _(Triaged at end-of-1.4 cycle. Full organized open-items + the fork-obligation checklist in the handoff doc `docs/sessions/contract-004-2026-06-18-phase1-trust-and-ports-handoff.md`.)_
