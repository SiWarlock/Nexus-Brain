...
**Track `spine` — Phase 2 (Ingestion + Redactor) IN PROGRESS.** Ingest front built against frozen Phase-1 contracts + `Fake*`: **2.1** discovery+classification (`dc5a9aa`), **2.2** anchor-aware chunking (docs `c10ea79` + code `434c46a`; vendored `CodeHierarchyNodeParser`, 4-of-12 langs with AST hierarchy + line-tiling fallback for the rest), **2.3** redactor catchable-set engine + fuzz gate (`723db12` ★SAFETY; recall 1.000 / FP 0.000 / git-SHA FP 0%). Suite **320** green; `/preflight` clean; LESSONS §19–§23. No Appendix-A field changed (frozen-contract guard held). **D-A5/A6 RESOLVED** (redaction 0.95/0.05 uniform). Cycled at WARN (impl 72%) before the heavy safety bundle — fresh full-runway team resumes.

**Next: the 2.4 + 2.S bundle** (owner's flagship bundle) — the `add` ingest orchestration (discover→classify→chunk→redact→manifest; idempotent; R-PARTIAL incl. a file-level malformed-content boundary) + the ★Task 2.S §14 INV-allowlist FULL runtime proof (D-A13, cardinal Key-safety-#4) + the real `StandaloneHost`. security-reviewer mandatory; **Acceptance(2)-gated** (Phase 2 does NOT close without 2.S) → `/phase-exit 2`. Last impl commit `434c46a` (+ this round-seal). Branch `track/spine`.

---

## Carry-forward to upcoming briefs

> _(Triaged at the spine Phase-2 round close-out 2026-06-21. From the contract→spine handoff set: D-A13/2.S KEPT (next brief), D-A14/4.2 KEPT (Phase-4 safety), O-FED + O-LANCE-BAKEOFF KEPT (phase carries); the Phase-2.3 redactor engine + fuzz-gate-wiring **DELETED** (landed `723db12`); the deferred-to-phase bundle's done items removed, the rest consolidated. Added: the 2.4-pipeline malformed-content boundary (next brief), the 2 owner directives, the parser-config + redactor-recalibration follow-ups. Triage: 1 deleted, 2 added, several consolidated. Hard cap ~7 (dense consolidations).)_

- **★SAFETY — NEXT BRIEF (the 2.4 + 2.S bundle; D-A13)** — Task **2.S**: the §14 INV-allowlist FULL runtime proof (every FS/git/session mutation routes via `HostPort.perform`) + the real `StandaloneHost`, landing WITH the first writer (2.4 manifest write). 1.4a seeded only the static AST-scan tripwire; this is the runtime per-mutator proof. **Acceptance(2)-gated** (Phase 2 does NOT close without it); security-reviewer mandatory. Bundle with 2.4 (owner bundle-aggressively directive). _(origin: 1.4a / contract→spine handoff)_
- **★ NEXT BRIEF (the 2.4 pipeline) — file-level malformed-content R-PARTIAL boundary + CONTENT sanitization** — pre-redaction NUL/C0/DEL currently hard-rejects at `ChunkDraft`/`Chunk` (`TextStr`) and would abort a whole file; 2.4 needs a per-file error boundary (skip/quarantine a bad file, never abort the repo) + Trojan-Source bidi/control sanitization (LESSON 14; deliberately kept OUT of the 2.3 secret engine). Pinned by `test_chunk_docs_nul_byte_raises_at_boundary`. _(origin: 2026-06-21 2.2a/2.3)_
...
- **Deferred follow-ups (lower priority; full detail in `docs/sessions/spine-001-…`)** — parser-config extension to the 12 langs (**JS+Rust first**, owner priority); redactor corpus-coverage gap (comma/brace-embedded + URL-under-sensitive-key → `ci/eval/redaction_fuzz/generator`; encoding-aware oracle/JWT/binary FLAG-1/2/3) + input-size cap at the persist sink + `_ENTROPY_MIN_BITS=4.0` margin; SecretStore real-keychain + log-scrub (Phase 2/3); 2.1 nested-`.gitignore` + v0 vocab gaps; 2.2a setext/`.rst`/`.adoc` + token-aware splitting + line:col code spans; 2.2b vendored-`assert`-under-`-O` + upstream re-sync; Phase-4 EvidenceType/IdKind pin (D-A11) + §14-ingress hardening + `get_file` realpath-containment (8.2).
- **Standing scaffolding/env items** — (a) spec-lint numeric-ID fix `392ed4f` → carry upstream at `/scaffold-upgrade`; (b) `requires-python "<3.13"` deliberate bump; (c) **D-A3 `/preflight` `mypy core`→`mypy .`** (HITL-deferred; implementers use `mypy .` interim); (d) **`pip-audit` install before the Phase-2-exit dep-audit gate** (new heavy deps: llama-index-core/tree-sitter + the 4 grammars + pathspec).
