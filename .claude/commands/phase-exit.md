...
| Dependency audit row (posture-gated, when present) | Run `pip-audit && (cd app && pnpm audit --prod && cargo audit)` once; new-vs-baseline summary (full output → `docs/audits/`) |
| Security review row (posture-gated, when present) | Resolve from `every-slice` — see the row's own text |
