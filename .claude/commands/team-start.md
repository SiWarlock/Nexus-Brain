...
- its **worktree path + branch** (default convention: `../project-brain-<track>` on branch `track/<track>`).
...
1. **`IMPLEMENTATION_PLAN.md`** — focused sections:
   ...
   - **The active phase within this track's phase set** (Phase <N> (tasks <N>.<M>) prefix) — task IDs + one-line topics. (Single-track plan → the active phase globally.)
...
```bash
git worktree add ../project-brain-<track> track/<track>   # or the base branch the Track map specifies
```
...
```
You are <track>-<area>-orchestrator on the Nexus Brain agent team.
...
```
You are <track>-<area>-implementer on the Nexus Brain agent team.
