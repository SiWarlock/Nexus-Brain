...
You routed each Step-9 item hot during the session, per the **canonical matrix in `docs/orchestrator-briefing.md`** (loaded at `/orchestrate-start` — don't re-copy it here). Verify each landed: grep the lesson title in `core/LESSONS.md` + its linked index row in `core/CLAUDE.md`; `git diff` any `ARCHITECTURE.md` edits; grep Carry-forward / the phase for routed TODOs; confirm any deferment was escalated. The **most-likely-to-slip** is *Completed work → ticked checkbox* — Step 3.
...
- Any hot-routed `core/LESSONS.md` + `core/CLAUDE.md` index additions that haven't ridden a slice commit yet
...
Then **push to origin (git@github.com:SiWarlock/Nexus-Brain.git) only**:
...
- _(production-grade, optional)_ a report-only `pip-audit && (cd app && pnpm audit --prod && cargo audit)` one-liner (new-vs-baseline) if run this round — the blocking run stays at `/phase-exit`
...
- **Pushing to the wrong remote.** Push to origin (git@github.com:SiWarlock/Nexus-Brain.git) only.
